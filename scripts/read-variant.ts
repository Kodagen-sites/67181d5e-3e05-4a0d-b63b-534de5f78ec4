...
export function extractRefSection(text: string, clean: string, section?: string): string {
  ...
  if (!section || !section.trim()) {
    ...
    const exKey = sectionHeads[0].text.split(/[—–:]/)[0].trim();
    const out: string[] = [`INDEX of references/${clean}.md — to read ONE section, copy a title below up to the "—" and pass it in quotes, e.g.  tsx /skill/scripts/read-variant.ts ${clean} "${exKey}"`];
  ...
  // Token-aware match. A short id ("g", "ft2") must hit a STANDALONE heading
  // token — not any substring — so "g" resolves to "Archetype G" and does NOT
  // spuriously match every "Archetype …" heading via the "g" inside "archetype"
  // (the bug that forced the agent into 4 retry probes). Multi-char descriptive
  // queries still match as a whole phrase ("structured hybrid" → "Archetype G").
  const splitTokens = (s: string) => s.split(/[\s—–:()-]+/).filter(Boolean);
  const wantTokens = splitTokens(want);
  const matches = (h: { text: string }) => {
    const ht = h.text.toLowerCase();
    // single id/word → must equal a standalone heading token;
    // multi-word query → whole-phrase substring.
    return wantTokens.length === 1
      ? splitTokens(ht).includes(wantTokens[0])
      : ht.includes(want);
  };
