...
async function generateImage(prompt: string, slot: string, aspect = "16:9"): Promise<ImageResp> {
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      if (isTerminalAssetError(String(err?.message ?? err)) || attempt === MAX_RETRIES) break;
...
/**
 * A failure that retrying CANNOT fix within this build: content-policy block,
 * or depleted credits / exhausted quota on BOTH providers (needs new credits
 * or a quota bump, not another request). Used both to break the per-image
 * retry loop AND to decide whether a whole-run failure should fail the build
 * or fall back to the runtime gradient/Unsplash cascade. Keep the patterns in
 * sync with the server messages in src/api/assets.ts.
 */
function isTerminalAssetError(msg: string): boolean {
  return /SAFETY|PROHIBITED_CONTENT|RESOURCE_EXHAUSTED|Resource has been exhausted|credits\s+are\s+depleted|Quota exceeded|both_providers_failed|\b429\b/i.test(
    msg,
  );
}
...
async function main() {
  ...
  async function worker(): Promise<void> {
    while (true) {
      ...
      try {
        ...
        failed++;
        if (isTerminalAssetError(String(err?.message ?? err))) quotaFailed++;
  ...
  if (failed > 0) {
    const unexpected = failed - quotaFailed;
    if (quotaFailed > 0) {
      // Graceful degradation — do NOT crash the build and do NOT signal "retry".
      // These slots failed on depleted credits / exhausted quota / content
      // policy, which a retry cannot fix. The generated site already resolves
      // any missing image slot through the runtime gradient→Unsplash cascade
      // in lib/image-fallback.ts, so the page still renders. Exit 0 so the
      // agent proceeds instead of re-running gen:images in a doomed loop.
      console.log(
        `⚠ IMAGE_FALLBACK: ${quotaFailed} slot(s) failed on depleted credits / exhausted quota / content policy — TERMINAL (a retry cannot fix these). Do NOT re-run gen:images. These slots render via the runtime gradient/Unsplash fallback. Continuing the build.`,
      );
    }
    if (unexpected > 0) {
      // Genuinely unexpected failures (network, malformed prompt, bug) — these
      // are worth surfacing as a non-zero exit so the agent investigates.
      console.error(`✗ ${unexpected} slot(s) failed for non-quota reasons — see errors above.`);
      process.exit(1);
    }
  }
}
