  ...
  return (
    <ul className="space-y-2 text-sm text-ink/70">
      {siteConfig.services.slice(0, limit).map((svc) => (
        <li key={svc.slug}>
          <Link href={`/services/${svc.slug}`} className="hover:text-ink transition-colors">
            {svc.name}
          </Link>
        </li>
      ))}
...
function ColHeading({ children }: { children: React.ReactNode }) {
  return (
    <h4 className="font-mono text-[11px] uppercase tracking-[0.2em] text-ink/45">{children}</h4>
  );
}
...
function BottomBar({ center = false }: { center?: boolean }) {
  return (
    <div
      className={`mt-16 pt-8 border-t border-ink/10 flex flex-col md:flex-row gap-4 text-xs text-ink/40 font-mono ${
    ...
    >
      ...
      <div className="flex gap-6">
        {LEGAL_LINKS.map((l) => (
          <Link key={l.href} href={l.href} className="hover:text-ink/70">{l.label}</Link>
        ))}
      </div>
    </div>
  );
...
function Shell({ children }: { children: React.ReactNode }) {
  return (
    <footer className="relative z-20 bg-bg border-t border-ink/10">
    ...
    <Shell>
      <div className="grid grid-cols-1 md:grid-cols-5 gap-10">
        <div className="md:col-span-2 space-y-4">
          <div className="font-display font-bold tracking-[0.15em] uppercase text-lg text-ink">
            {siteConfig.company.name}
          </div>
          <p className="text-sm text-ink/55 leading-relaxed max-w-sm">
            {siteConfig.company.description}
          </p>
          <SocialLinks socials={siteConfig.socials} className="pt-4 gap-4 text-ink/70" iconClassName="h-5 w-5" />
        </div>
    ...
    <Shell>
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        <div className="lg:col-span-5 space-y-5">
          <div className="font-display font-bold text-3xl text-ink">{siteConfig.company.name}</div>
          <p className="text-base text-ink/65 leading-relaxed max-w-md">
            {siteConfig.company.description}
          </p>
          <SocialLinks socials={siteConfig.socials} className="pt-2 gap-4 text-ink/70" iconClassName="h-5 w-5" />
        </div>
  ...
  return (
    <Shell>
      <div className="flex flex-col items-center text-center gap-6">
        <div className="font-display font-bold tracking-[0.2em] uppercase text-xl text-ink">
          {siteConfig.company.name}
        </div>
        <p className="text-sm text-ink/55 max-w-lg leading-relaxed">{siteConfig.company.description}</p>
        <nav className="flex flex-wrap justify-center gap-x-8 gap-y-2 text-sm text-ink/70">
          {[{ href: "/services", label: "Services" }, ...COMPANY_LINKS].map((l) => (
            <Link key={l.href} href={l.href} className="hover:text-ink">{l.label}</Link>
          ))}
        </nav>
        <SocialLinks socials={siteConfig.socials} className="gap-5 text-ink/70" iconClassName="h-5 w-5" />
      </div>
      <BottomBar center />
    </Shell>
  );
...
function FooterFT5() {
  ...
  return (
    <Shell>
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6 pb-12 border-b border-ink/10">
        <h3 className="font-display font-semibold text-2xl md:text-3xl text-ink max-w-xl leading-tight">
          {siteConfig.company.description}
        </h3>
        <Link
          href={primaryCta.href}
          className="inline-flex items-center gap-2 self-start rounded-full bg-primary px-6 py-3 text-sm font-medium text-ink hover:opacity-90 transition"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-10 pt-12">
        <div className="space-y-4">
          <div className="font-display font-bold tracking-[0.15em] uppercase text-ink">{siteConfig.company.name}</div>
          <SocialLinks socials={siteConfig.socials} className="gap-4 text-ink/70" iconClassName="h-5 w-5" />
        </div>
